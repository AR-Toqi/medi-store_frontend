    ...
    <div className="container mx-auto p-4 md:p-8 max-w-7xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        ...
        <div className="flex items-center gap-3">
          <Card className="border-none shadow-md bg-white px-5 py-3 rounded-2xl flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-black text-slate-800">{meta?.total || 0}</p>
            </div>
          </Card>
        </div>
      </div>

      <Card className="border-none shadow-2xl shadow-slate-200/50 overflow-hidden bg-white/90 backdrop-blur-md rounded-3xl">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <Select value={status} onValueChange={(val) => { setStatus(val); setPage(1); }}>
              <SelectTrigger className="w-full sm:w-[180px] h-11 rounded-xl bg-white border-slate-200 font-bold">
          ...
          <div className="text-xs font-black text-slate-400 bg-white px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-2">
            Showing <span className="text-[#00bc8c]">{orders.length}</span> results for <span className="text-slate-800 uppercase">{status}</span>
          </div>
        </div>
                    ...
                    return (
                      <TableRow key={order.id} className="group border-slate-50">
                        <TableCell className="px-6 py-5">
                           <div className="flex items-center gap-3">
                              <span className="h-8 w-8 rounded-lg bg-slate-100 flex items-center justify-center font-mono text-[10px] font-bold text-slate-500">
                                #
                              </span>
                              <div>
                                <p className="font-black text-slate-900 group-hover:text-[#00bc8c] transition-colors">
                                  {order.id.slice(0, 12)}...
                                </p>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Customer ID: {order.customerId.slice(0, 8)}</p>
                              </div>
                           </div>
                        </TableCell>
                        <TableCell className="px-6 py-5">
                        ...
                        <TableCell className="px-6 py-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                             <Link href={`/orders/${order.id}`}>
                               <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-slate-100 cursor-pointer">
                             ...
                             >
                                <SelectTrigger className="w-32 h-9 rounded-xl bg-slate-100 border-transparent font-bold text-[11px] hover:bg-slate-200 transition-colors">
