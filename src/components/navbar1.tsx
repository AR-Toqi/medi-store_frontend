...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-white/80 dark:bg-slate-900/80 backdrop-blur-md supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-slate-900/60 shadow-sm dark:shadow-slate-950/20 dark:border-slate-800">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between gap-4">
        ...
        <div className="hidden lg:flex items-center gap-8 shrink-0">
          <nav className="flex items-center gap-8 text-[13px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
          ...
          <div className="flex items-center gap-3">
            {isLoading ? (
              <div className="flex items-center gap-3 p-1.5 pr-4 rounded-2xl border border-slate-100/50 dark:border-slate-700/50 animate-pulse">
                <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800" />
                <div className="flex flex-col gap-1.5">
                  <div className="w-20 h-2.5 bg-slate-100 dark:bg-slate-800 rounded" />
                  <div className="w-12 h-2 bg-slate-50 dark:bg-slate-800 rounded" />
                </div>
              </div>
            ) : user ? (
              <Link href="/profile" className="flex items-center gap-3 p-1.5 pr-4 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-2xl transition-all border border-transparent hover:border-slate-100 dark:hover:border-slate-700 group">
                <div className="w-10 h-10 rounded-xl overflow-hidden bg-[#00bc8c]/10 flex items-center justify-center border-2 border-white dark:border-slate-800 shadow-sm group-hover:shadow-md transition-all">
                  {user.image ? (
                ...
                <div className="flex flex-col">
                  <span className="text-[14px] font-black text-slate-800 dark:text-slate-100 leading-none">{user.name}</span>
          ...
          {/* Cart Section */}
          <CartDrawer>
            <button className="relative p-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-2xl transition-all group active:scale-90">
              <ShoppingCart className="w-7 h-7 text-slate-600 dark:text-slate-300 group-hover:text-[#00bc8c] transition-colors duration-300" strokeWidth={2} />
              {cartCount > 0 && (
                <span className="absolute top-1 right-1 bg-red-500 text-white text-[10px] font-black min-w-5 h-5 px-1 flex items-center justify-center rounded-full border-2 border-white dark:border-slate-900 shadow-sm">
                  {cartCount}
                </span>
              )}
            </button>
        ...
        <div className="lg:hidden flex items-center gap-4">
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px] rounded-l-[2rem] dark:bg-slate-900 dark:border-slate-800">
              ...
              <div className="flex flex-col gap-8 py-4">
                 <NavSearch />
                 <nav className="flex flex-col gap-6 text-lg font-bold dark:text-slate-200">
